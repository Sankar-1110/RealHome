...
export default function Create_list() {
  ...
  const handleSubmit=async(e)=>{
    e.preventDefault();
    try{
    if(formData.imageUrls.length<1){  setError('Choose atleast one image')
      setTimeout(() => {
        setError(false);
      },3000 );;
      return;
    ...
    <div className='flex flex-col gap-4 flex-wrap  p-4 w-full sm:w-1/2'>
      ...
      <div>
        {error ? (
          ...
          <div className='flex flex-col gap-2 bg-slate-200 border border-slate-500 rounded-lg px-2'>
            {formData.imageUrls.map((url,index) => (
            <div key={url} className='flex justify-between'>
              <img src={url} alt='uploaded images' className='my-1 w-16' /> 
              <button type='button' onClick={()=>handleDeleteImg(index)} className='text-red-600 px-2  hover:text-red-700'>Delete</button>
              </div>
            ))}
          </div>
        )}
